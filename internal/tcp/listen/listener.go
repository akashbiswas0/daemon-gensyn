...
var (
	...
	// peerIDResolver maps a remote transport address to the canonical peer public key when known.
	peerIDResolver func(net.Addr) string
)
...
func SetupNetworkStack(yggCore *core.Core, tcpPort int, mcpRouterUrl string, a2aURL string) {
	...
	peerIDResolver = func(addr net.Addr) string {
		host, _, _ := net.SplitHostPort(addr.String())
		ip := net.ParseIP(host)
		if ip == nil {
			return ""
		}
		resolveKey := func(key []byte) string {
			nodeAddr := address.AddrForKey(key)
			if nodeAddr == nil {
				return ""
			}
			if net.IP(nodeAddr[:]).Equal(ip) {
				return hex.EncodeToString(key)
			}
			return ""
		}
		if peerID := resolveKey(yggCore.PublicKey()); peerID != "" {
			return peerID
		}
		for _, peer := range yggCore.GetPeers() {
			if peerID := resolveKey(peer.Key); peerID != "" {
				return peerID
			}
		}
		for _, entry := range yggCore.GetTree() {
			if peerID := resolveKey(entry.Key); peerID != "" {
				return peerID
			}
		}
		for _, session := range yggCore.GetSessions() {
			if peerID := resolveKey(session.Key); peerID != "" {
				return peerID
			}
		}
		return ""
...
func resolvePeerID(addr net.Addr) string {
	if peerIDResolver != nil {
		if peerID := peerIDResolver(addr); peerID != "" {
			return peerID
		}
	}
...
func handleTCPConn(conn net.Conn, mux *Multiplexer) {
	...
	fromPeerId := resolvePeerID(conn.RemoteAddr())
	log.Printf("Connection from peer %s", fromPeerId)
	...
	for {
		...
		if length > api.MaxMessageSize {
			log.Printf("Message size %d from peer %s exceeds maximum %d, closing connection",
				length, fromPeerId, api.MaxMessageSize)
			return
		}
